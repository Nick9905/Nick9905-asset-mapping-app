...
def all_data_view_page():
  ...
  if view_mode == "财务系统明细":
      ...
      st.markdown("---")
      st.subheader("📊 财务系统-资产明细账")
      st.info("💡 选择下方表格中的任意行，查看对应的实物资产信息")
      ...
      if len(df) > 0:
          ...
          # 选择行查看对应关系
          st.subheader("🔍 选择资产查看对应关系")
          if len(df) > 0:
              selected_index = st.selectbox(
                  "选择要查看的财务资产",
                  range(len(df)),
                  format_func=lambda x: f"{df.iloc[x]['财务系统编号']} - {df.iloc[x]['资产名称']}"
              )
              
              if st.button("查看对应的实物资产", type="primary"):
                  selected_financial = df.iloc[selected_index]
                  ...
                  if corresponding_asset:
                      ...
                      with col2:
                          ...
                          st.write(f"**部门**：{corresponding_asset['存放部门']}")
      ...
      st.markdown("---")
      st.subheader("📋 实物台账明细")
      st.info("💡 选择下方表格中的任意行，查看对应的财务系统信息")
      ...
      if len(df) > 0:
          ...
          selected_index = st.selectbox(
              "选择要查看的实物资产",
              range(len(df)),
              format_func=lambda x: f"{df.iloc[x]['固定资产编号']} - {df.iloc[x]['固定资产名称']}"
          )
          
          if st.button("查看对应的财务资产", type="primary"):
              selected_physical = df.iloc[selected_index]
              ...
              if corresponding_asset:
                  ...
                  with col1:
                      ...
                      st.write(f"**部门**：{selected_physical['存放部门']}")
