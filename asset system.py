...
GITHUB_REPO = Nick9905Nick9905-asset-mapping-app  # 请修改这里！！！
GITHUB_BRANCH = main
def save_data_to_github(data, filename)
    ...
    try
        ...
        # GitHub API URL
        url = fhttpsapi.github.comrepos{GITHUB_REPO}contents{filename}
...
def convert_to_number(value, default=0)
    ...
    try
        ...
        if isinstance(value, str)
            ...
            # 处理常见的文本情况
            if cleaned_value.lower() in ['', '-', 'nan', 'null', 'none', '无', '空', 'na', '#na', '#value!',
                                         '#div0!']
                return default
            ...
            cleaned_value = cleaned_value.replace(',', '').replace('¥', '').replace('￥', '').replace('$', '').replace(
                '€', '')
            cleaned_value = cleaned_value.replace('，', '').replace(' ', '').replace('t', '').replace('n', '')
                    ...
                    if 资产价值 in df_current.columns
                        try
                            ...
                            with st.expander(🚨 错误详情)
                                st.code(f错误类型 {type(e).__name__}n错误信息 {str(e)})
                                if len(df_current)  0
                ...
                with col2
                    ...
                    if st.button(🗑️ 删除指定编号, key=financial_code_delete)
                        if delete_codes.strip()
                            codes_to_delete = [code.strip() for code in delete_codes.split('n') if code.strip()]
                            original_count = len(current_financial)
                            filtered_data = [
                                record for record in current_financial
                ...
                with col2
                    if st.button(📥 导出当前数据, use_container_width=True)
                        ...
                        st.download_button(
                            ...
                            file_name=f财务数据_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx,
                    ...
                    if 固定资产原值 in df_current.columns
                        try
                            ...
                            if duplicate_count  0
                                ...
                                caption_text = f去重后金额（删除{duplicate_count}条重复）
                            ...
                            if duplicate_count  0
                                st.metric(固定资产原值总计, f¥{total_value_dedup,.2f})
                                st.caption(f去重后金额（删除{duplicate_count}条重复）)
                            else
                                st.metric(固定资产原值总计, f¥{total_value_raw,.2f})
                                st.caption(无重复记录)
        ...
        if current_mapping  
            ...
            with st.expander(🗑️ 映射关系快速删除, expanded=False)
                ...
                with col2
                    ...
                    delete_codes = st.text_area(
                        f输入要删除的{'财务编号' if delete_type == '按财务编号' else '实物编码'}（每行一个）,
                        height=100,
                ...
                with col2
                    if st.button(📥 导出当前数据, use_container_width=True)
                        ...
                        st.download_button(
                            label=⬇️ 下载Excel文件,
                            data=output.getvalue(),
                            file_name=f映射数据_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx,
                            mime=applicationvnd.openxmlformats-officedocument.spreadsheetml.sheet
                        )
    ...
    with tab4
        ...
        with col1
            ...
            st.metric(
                ...
                delta=f总价值 {sum(safe_convert_to_float(record.get('资产价值', 0)) for record in financial_data),.2f} if financial_data else 无数据
            )
        ...
        with col2
            ...
            st.metric(
                ...
                delta=f总价值 {sum(safe_convert_to_float(record.get('资产价值', 0)) for record in physical_data),.2f} if physical_data else 无数据
            )
            ...
            if st.button(📥 创建完整数据备份, use_container_width=True)
                try
                    ...
                    st.download_button(
                        label=⬇️ 下载完整备份文件,
                        data=output.getvalue(),
                        file_name=f完整数据备份_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx,
                        mime=applicationvnd.openxmlformats-officedocument.spreadsheetml.sheet
                    )
        ...
        if delete_options
            ...
            with col1
                confirm_delete = st.checkbox(
                    f我确认要删除选中的 {len(delete_options)} 类数据,
...
def mapping_query_page()
    ...
    if query_type == 按资产编号选择查询
        ...
        with col1
            selected_asset_number = st.selectbox(
                f选择资产编号 (共 {len(sorted_asset_numbers)} 个),
                ...
                for i, full_code in enumerate(sorted(full_codes), 1)
                    ...
                    if financial_record
                        ...
                        with st.expander(f📊 资产 #{i} {full_code} - {financial_record.get('资产名称', '')},
                            ...
                            with col1
                                st.info(f完整编号 {financial_record.get('资产编号+序号', '')})
                                st.info(f资产名称 {financial_record.get('资产名称', '')})
                                st.info(f资产分类 {financial_record.get('资产分类', '')})

                            with col2
                                financial_value = safe_get_value(financial_record, 资产价值)
                                st.info(f资产价值 ¥{financial_value,.2f})
                                st.info(f所属部门 {financial_record.get('部门名称', '')})
                                st.info(f保管人 {financial_record.get('保管人', '')})
    ...
    if query_type == 按资产编号+序号查询
            ...
            if financial_code
                ...
                if financial_record
                    ...
                    with st.expander(📊 财务资产详情, expanded=True)
                        col1, col2 = st.columns(2)
                        with col1
                            st.info(f资产编号 {financial_record.get('资产编号+序号', '')})
                            st.info(f资产名称 {financial_record.get('资产名称', '')})
                            st.info(f资产分类 {financial_record.get('资产分类', '')})
                        with col2
                            financial_value = safe_get_value(financial_record, 资产价值)
                        ...
                        for i, physical_code in enumerate(physical_codes, 1)
                            ...
                            if physical_record
                                ...
                                with st.expander(f📋 实物资产详情 #{i} - {physical_code}, expanded=True)
                                    col1, col2 = st.columns(2)
                                    with col1
                                        st.info(f资产编号 {physical_record.get('固定资产编码', '')})
                                        st.info(f资产名称 {physical_record.get('固定资产名称', '')})
                                        st.info(f资产类型 {physical_record.get('固定资产类型', '')})
                                    with col2
                                        physical_value = safe_get_value(physical_record, 资产价值)
                                        st.info(f资产价值 ¥{physical_value,.2f})
                                        st.info(f存放部门 {physical_record.get('存放部门', '')})
                                        st.info(f使用状态 {physical_record.get('使用状态', '')})
                ...
                if financial_results
                    for record in financial_results[10]  # 限制显示前10条
                        with st.expander(f💰 {record.get('资产名称', '')} - {record.get('资产编号+序号', '')})
                            st.write(f资产分类 {record.get('资产分类', '')})
                            asset_value = safe_get_value(record, 资产价值)
                            st.write(f资产价值 ¥{asset_value,.2f})
                            st.write(f所属部门 {record.get('部门名称', '')})
                ...
                if physical_results
                    for record in physical_results[10]  # 限制显示前10条
                        with st.expander(f📦 {record.get('固定资产名称', '')} - {record.get('固定资产编码', '')})
                            st.write(f资产类型 {record.get('固定资产类型', '')})
                            asset_value = safe_get_value(record, 资产价值)
                            st.write(f资产价值 ¥{asset_value,.2f})
                            st.write(f存放部门 {record.get('存放部门', '')})
                            st.write(f使用状态 {record.get('使用状态', '')})
        ...
        batch_input = st.text_area(
            请输入要查询的编号（每行一个）,
            placeholder=FS001nFS002nPA001nPA002,
            height=150
        )
        ...
        if batch_input and st.button(开始批量查询)
            codes = [code.strip() for code in batch_input.split('n') if code.strip()]
                    ...
                    if st.button(📥 导出查询结果)
                        try
                            ...
                            st.download_button(
                                label=下载Excel文件,
                                data=output,
                                file_name=f批量查询结果_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx,
                                mime=applicationvnd.openxmlformats-officedocument.spreadsheetml.sheet
                            )
                        except Exception as e
                            st.error(f导出失败 {str(e)})
...
def data_statistics_page()
    ...
    with tab_summary
        ...
        with col1
            ...
            st.caption(f已匹配 {matched_financial,})
        ...
        with col2
            ...
            if duplicate_count  0
                st.caption(f原始 {original_count,}  去重 {duplicate_count})
            else
                st.caption(f已匹配 {matched_physical,})
        ...
        with col1
            ...
            st.caption(f未匹配 {unmatched_financial,} 项)
        ...
        with col2
            ...
            st.caption(f未匹配 {unmatched_physical,} 项)
    ...
    with tab_analysis
        ...
        with tab_analysis
            ...
            if not all([financial_data, physical_data, mapping_data])
                ...
                with col_status1
                    st.info(f原值差异状态 {get_status_emoji(total_original_diff)})
                with col_status2
                    st.info(f折旧差异状态 {get_status_emoji(total_depreciation_diff)})
                with col_status3
                    st.info(f净额差异状态 {get_status_emoji(total_net_diff)})
                ...
                if st.button(📥 导出差异分析报告, key=export_analysis)
                    ...
                    st.download_button(
                        label=💾 下载差异分析报告 CSV,
                        data=csv,
                        file_name=f资产差异分析报告_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv,
                        mime=textcsv
                    )
...
def all_data_view_page()
    ...
    if view_mode == 对应关系汇总
        ...
        if mapping_summary
            ...
            st.info(f共 {len(filtered_df)} 条记录（总映射关系 {len(df)} 条）)
            ...
            if st.button(📥 导出为Excel)
                try
                    ...
                    st.download_button(
                        label=下载Excel文件,
                        data=output,
                        file_name=f资产对应关系汇总_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx,
                        mime=applicationvnd.openxmlformats-officedocument.spreadsheetml.sheet
                    )
                except Exception as e
                    st.error(f导出失败 {str(e)})
        ...
        st.info(f共 {len(filtered_df)} 条记录（总财务资产 {len(df)} 条）)
        ...
        with col3
            ...
            try
                ...
                if valid_count  0
                    ...
                    st.caption(f有效记录 {valid_count}{len(filtered_df)} ({success_rate.1f}%))
        ...
        st.info(f共 {len(filtered_df)} 条记录（总实物资产 {len(df)} 条）)
        ...
        try
            ...
            if duplicate_count  0
                ...
                caption_text = f已去重 ({duplicate_count}条)
                if has_accounting_field and non_accounting_dedup_count  0
                    caption_text += f  已排除{non_accounting_dedup_count}条非核算
                st.caption(caption_text)
            else
                    ...
                    if st.button(📥 导出重复记录, key=export_duplicates)
                        try
                            ...
                            st.download_button(
                                label=下载重复记录Excel,
                                data=output,
                                file_name=f重复记录分析_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx,
                                mime=applicationvnd.openxmlformats-officedocument.spreadsheetml.sheet,
            ...
            with st.expander(🔧 数据异常分析, expanded=False)
                ...
                if error_records
                    ...
                    if len(error_records)  10
                        st.info(f显示前10条，共{len(error_records)}条异常记录)
        ...
        with tab1
            if not financial_data
                ...
                if unmatched_financial  # ✅ 添加：判断是否有未匹配数据
                    ...
                    st.info(f共 {len(df)} 条未匹配财务资产（总计 {len(financial_data)} 条）)
                    ...
                    with col2
                        ...
                        with col3
                            ...
                            try
                                ...
                                st.error(f计算错误 {str(e)})

                        with col4
                            # 计算资产净值总计 - 财务系统，使用资产净额字段
                            try
                                total_net_value = 0.0
                                valid_net_count = 0
                                zero_net_count = 0
        ...
        with tab2
            if not physical_data
                ...
                if unmatched_physical  # ✅ 添加：判断是否有未匹配数据
                    ...
                    st.info(f共 {len(df)} 条未匹配实物资产（总计 {len(physical_data)} 条）)
                    ...
                    if st.button(📥 导出未匹配实物资产, key=export_unmatched_physical)
                        try
                            ...
                            st.download_button(
                                label=下载Excel文件,
                                data=output,
                                file_name=f未匹配实物资产_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx,
                                mime=applicationvnd.openxmlformats-officedocument.spreadsheetml.sheet,
