...
def create_mapping_index(mapping_data):
    ...
    for record in mapping_data:
        financial_code = record.get("财务系统编号", "")
        physical_code = record.get("固定资产编号", "")

        if financial_code and physical_code:
            financial_code = str(financial_code)
            physical_code = str(physical_code)
    ...
    if query_type == "按财务系统编号查询":
        ...
        if financial_code:
            ...
            if financial_record:
                ...
                with st.expander("📊 财务资产详情", expanded=True):
                    ...
                    with col2:
                        asset_value = safe_get_value(financial_record, "资产价值")
                        st.info(f"**资产价值**: ¥{asset_value:,.2f}")
                        st.info(f"**所属部门**: {financial_record.get('部门名称', '')}")
                        st.info(f"**保管人**: {financial_record.get('保管人', '')}")

                # 查找对应的实物资产
                physical_code = f_to_p_mapping.get(str(financial_code))
        ...
        if physical_code:
            ...
            if physical_record:
                ...
                # 查找对应的财务资产
                financial_code = p_to_f_mapping.get(str(physical_code))
                ...
                if financial_results:
                    for record in financial_results[:10]:  # 限制显示前10条
                        with st.expander(f"💰 {record.get('资产名称', '')} - {record.get('财务系统编号', '')}"):
                            ...
                            # 检查是否有对应的实物资产
                            physical_code = f_to_p_mapping.get(str(record.get('财务系统编号', '')))
                            if physical_code:
                ...
                if physical_results:
                    for record in physical_results[:10]:  # 限制显示前10条
                        with st.expander(f"📦 {record.get('固定资产名称', '')} - {record.get('固定资产编号', '')}"):
                            ...
                            # 检查是否有对应的财务资产
                            financial_code = p_to_f_mapping.get(str(record.get('固定资产编号', '')))
                            if financial_code:
            ...
            if codes:
                ...
                for code in codes:
                    if query_mode == "财务系统编号":
                        ...
                        if financial_record:
                            physical_code = f_to_p_mapping.get(str(code))
                            ...
                            results.append({
                                ...
                                "实物资产名称": physical_record.get('固定资产名称',
                                                                    '') if physical_record else "未映射",
                                "实物资产价值": safe_get_value(physical_record, '资产价值') if physical_record else 0,
                        ...
                        if physical_record:
                            financial_code = p_to_f_mapping.get(str(code))
                            ...
                            results.append({
                                ...
                                "财务资产价值": safe_get_value(financial_record, '资产价值') if financial_record else 0,
                ...
                def data_statistics_page():
                    ...
                    # 计算总价值
                    financial_total_value = sum(safe_get_value(record, "资产价值") for record in financial_data)
                    physical_total_value = sum(safe_get_value(record, "资产价值") for record in physical_data)

                    # 计算已映射资产价值
                    mapped_financial_value = sum(
                        safe_get_value(financial_index.get(code, {}), "资产价值")
                        for code in f_to_p_mapping.keys()
                    )
                    mapped_physical_value = sum(
                        safe_get_value(physical_index.get(code, {}), "资产价值")
                    ...
                    if value_differences:
                        ...
                        with st.expander("查看价值差异明细"):
                            ...
                            df_diff = df_diff.reindex(df_diff['差异金额'].abs().sort_values(ascending=False).index)
                ...
                def all_data_view_page():
                    ...
                    if view_mode == "对应关系汇总":
                        ...
                        if mapping_summary:
                            ...
                            col1, col2, col3 = st.columns(3)
                            with col1:
                                all_depts = list(set(df["财务部门"].unique()) | set(df["实物部门"].unique()))
                                all_depts = [dept for dept in all_depts if dept]  # 过滤空值
                                dept_filter = st.selectbox("按部门筛选", ["全部"] + sorted(all_depts))
                            with col2:
                        ...
                        # 添加匹配状态列
                        df["匹配状态"] = df["财务系统编号"].astype(str).apply(
                            lambda x: "已匹配" if x in f_to_p_mapping else "未匹配")
                        ...
                        if search_term:
                            filtered_df = filtered_df[
                                ...
                                filtered_df["财务系统编号"].astype(str).str.contains(search_term, case=False, na=False)
                                ]
                        ...
                        default_columns = ["财务系统编号", "资产名称", "资产分类", "资产价值", "部门名称", "保管人",
                                           "匹配状态"]
                        ...
                        # 添加匹配状态列
                        df["匹配状态"] = df["固定资产编号"].astype(str).apply(
                            lambda x: "已匹配" if x in p_to_f_mapping else "未匹配")
                        ...
                        if search_term:
                            filtered_df = filtered_df[
                                filtered_df["固定资产名称"].astype(str).str.contains(search_term, case=False,
                                                                                     na=False) |
                                filtered_df["固定资产编号"].astype(str).str.contains(search_term, case=False, na=False)
                                ]
                        ...
                        # 选择要显示的列
                        available_columns = list(filtered_df.columns)
                        default_columns = ["固定资产编号", "固定资产名称", "固定资产类型", "资产价值", "存放部门",
                                           "保管人", "使用状态", "匹配状态"]
                        display_columns = [col for col in default_columns if col in available_columns]
                        ...
                        with tab1:
                            unmatched_financial = [f for f in financial_data if
                                                   str(f.get("财务系统编号", "")) not in f_to_p_mapping]
                            if unmatched_financial:
                                ...
                                default_columns = ["财务系统编号", "资产名称", "资产分类", "资产价值", "部门名称",
                                                   "保管人"]
                                display_columns = [col for col in default_columns if col in available_columns]
                        ...
                        with tab2:
                            unmatched_physical = [p for p in physical_data if
                                                  str(p.get("固定资产编号", "")) not in p_to_f_mapping]
                            if unmatched_physical:
                                ...
                                # 选择要显示的列
                                available_columns = list(df.columns)
                                default_columns = ["固定资产编号", "固定资产名称", "固定资产类型", "资产价值",
                                                   "存放部门", "保管人", "使用状态"]
                                display_columns = [col for col in default_columns if col in available_columns]
